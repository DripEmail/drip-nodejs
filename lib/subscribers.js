...
module.exports = {
  ...
  updateBatchSubscribers: function (accountId, payload, callback) {
    var subscribers = payload && payload.batches && payload.batches.subscribers || [];
    var batchSize = 1000;
    var batches = [];
    ...
    // Internal callback to collect results from each batch posting
    function postCallback(error, response, body) {
      ...
      if(responses.length === batches.length) {
        ...
        callback(errors, responses, bodies);
      }
    }
  }
}
