...
module.exports = {
   ...
   * Fetch a workflow
   *
   * @param {number} workflow_id - Required. The workflow's id
   * @param {callback} callback - An optional callback
   * @returns {promise}
   */
  ...
  removeFromWorkflow(workflowId, idOrEmail, callback) {
    const encodedIdOrEmail = helpers.escapeIdOrEmail(idOrEmail);
    return this.delete(`${this.accountId}/workflows/${workflowId}/subscribers/${encodedIdOrEmail}`, {}, callback);
  }
};
